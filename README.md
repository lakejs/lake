...
Note: `lake.min.js` is not built with CodeMirror, so addtioanaly including `codemirror.min.js` to your page is needed. But if you do not need the code block feature, you do not have to include CodeMirror, just set the global variable to null. To find out more, take a look at the [IIFE example](https://github.com/lakejs/lake/blob/master/examples/iife.html) and [Rollup configuration](https://github.com/lakejs/lake/blob/master/rollup.config.mjs).
